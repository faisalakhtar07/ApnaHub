...
import { notificationsApi, userAuthApi } from "../lib/api";
...
export default function NotificationBell() {
  ...
  const prevUnreadRef = useRef(null); // null = haven't loaded yet, so we don't play a sound on first load

  const poll = async () => {
    if (!userAuthApi.isLoggedIn()) return;
    try {
    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 mt-2 w-80 max-h-96 overflow-y-auto bg-white dark:bg-[#131B2E] border border-slate-100 dark:border-white/10 rounded-2xl shadow-xl z-50 animate-fadeUp">
